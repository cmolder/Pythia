...
CHAMP_PATH="$(pwd)"
PREFETCH="scooby"
OUTPUT_DIR="${CHAMP_PATH}/out/scooby_bw_feature_shift"
...
while read TRACE; do
    ...
    echo "./experiments/champsim.py run ${TRACE_PATH}/${TRACE} --l2c-pref ${PREFETCH} --results-dir ${OUTPUT_DIR} --run-name ${TRACE} --extra-knobs \"‘--scooby_bw_feature_shift=true’\"" >> ${SCRIPT_FILE}
    chmod +x ${SCRIPT_FILE}
