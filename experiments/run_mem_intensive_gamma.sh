...
PREFETCH="scooby"
GAMMA=${1}
OUTPUT_DIR="${CHAMP_PATH}/out/scooby_gamma_${GAMMA}"
...
while read TRACE; do
    ...
    echo "./experiments/champsim.py run ${TRACE_PATH}/${TRACE} --l2c-pref ${PREFETCH} --results-dir ${OUTPUT_DIR} --run-name ${TRACE} --extra-knobs \"‘--scooby_gamma=0.${GAMMA}’\"" >> ${SCRIPT_FILE}
    chmod +x ${SCRIPT_FILE}
