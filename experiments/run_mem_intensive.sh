...
CHAMP_PATH="$(pwd)"
PREFETCH=${1}
OUTPUT_DIR="${CHAMP_PATH}/out/${PREFETCH}"
...
while read TRACE; do
    ...
    echo "./experiments/champsim.py run ${TRACE_PATH}/${TRACE} --l2c-pref ${PREFETCH} --results-dir ${OUTPUT_DIR} --run-name ${TRACE}" >> ${SCRIPT_FILE}
    chmod +x ${SCRIPT_FILE}
