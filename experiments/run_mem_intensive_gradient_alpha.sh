...
PREFETCH="scooby"
ALPHA=${1}
OUTPUT_DIR="${CHAMP_PATH}/out/scooby_gradient_alpha_${ALPHA}"
...
while read TRACE; do
    ...
    echo "./experiments/champsim.py run ${TRACE_PATH}/${TRACE} --l2c-pref ${PREFETCH} --results-dir ${OUTPUT_DIR} --run-name ${TRACE} --extra-knobs \"‘--scooby_gamma=0.0 --scooby_alpha=0.${ALPHA} --le_featurewise_selective_update=true’\"" >> ${SCRIPT_FILE}
    chmod +x ${SCRIPT_FILE}
