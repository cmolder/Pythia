...
class ChampsimFile():
    ...
    Inherited by:
        ChampsimResultsFile: Paths for results paths (.txt)
            ChampsimStatsFile: Paths for per-PC/address statistics (_(l1d|l2c|llc).txt)
        ChampsimTraceFile: Paths for traces (.trace.gz)
    """
    def __init__(self, path: str):
        self.path = path
        self.full_trace = \
            ChampsimFile._get_full_trace(path)  # Trace and simpoint together
        self.trace, self.simpoint = \
            ChampsimFile._get_trace(path)       # Trace and simpoint separately
...
class ChampsimResultsFile(ChampsimFile):
    ...
    def __init__(self, path: str):
        ...
        self.l1_prefetcher, self.l2_prefetcher, self.llc_prefetcher = \
            ChampsimResultsFile._get_prefetcher_names(path) # Prefetcher names (l1, l2, llc)
        self.l2_prefetcher_degree, self.llc_prefetcher_degree = \
            ChampsimResultsFile._get_prefetcher_degrees(path) # Prefetcher degrees (l2, llc)
    ...
    def read(self) -> Optional[dict]:
        ...
        expected_keys = ('ipc', 'dram_bw_epochs', 
                         'L1D_useful', 'L2C_useful', 
                         'LLC_useful', 'L1D_useless', 
                         'L2C_useless', 'LLC_useless',
                         'L1D_issued_prefetches', 'L2C_issued_prefetches', 
                         'LLC_issued_prefetches', 'L1D_total_miss', 
                         'L2C_total_miss', 'LLC_total_miss', 
                         'L1D_load_miss', 'L2C_load_miss', 
                         'LLC_load_miss', 'L1D_rfo_miss', 
                         'L2C_rfo_miss', 'LLC_rfo_miss',
                         'seed', 'kilo_inst')
        data = {}
    ...
    def get_all_variants(self) -> Tuple:
        """Get the key for the variant properties (degree, Pythia variables, etc.),
        mainly for indexing inside a Directory.
        """
        variants = {
    ...
    def _get_pythia_level_threshold(path: str) -> Optional[float]:
        ...
        """
        if not any(p == 'scooby' for p in ChampsimResultsFile._get_prefetcher_names(path)):
            return None

        path = os.path.basename(path)
        if 'threshold' in path:
            return float(path[path.index('threshold'):]
                             .split('_')[1]
                             .split('.')[0])
        return None
...
class ChampsimStatsFile(ChampsimResultsFile):
    ...
    def __init__(self, path: str, index: str = 'pc'):
        assert (path.endswith('_l1d.txt') or
                ...
                'Wrong file extension for ChampsimStatsFile, should be _(l1d|l2c|llc).txt'
        super().__init__(path)
        
        self.level = path.split('_')[-1].split('.')[0] # l1d, l2c, llc, inferred from path
        self.index = index
...
class ChampsimResultsDirectory(ChampsimDirectory):
    """A directory containing ChampSim results files (ChampsimResultFile).
