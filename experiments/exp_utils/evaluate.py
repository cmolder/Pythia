...
def generate_csv(results_dir, output_file, dry_run=False):
    ...
    for path in glob.glob(os.path.join(results_dir, '*.txt')):
        ...
        traces[full_trace][(l1d_pref, l2c_pref, llc_pref)][(l2c_pref_deg, llc_pref_deg)] = path
        ...
        for tr in traces:
            ...
            for pf in traces[tr]:
                for d in traces[tr][pf]:
                    ...
                    row = get_statistics(traces[tr][pf][d], baseline_path=traces[tr][('no', 'no', 'no')][(None,), (None,)])         
