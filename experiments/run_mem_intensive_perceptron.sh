...
CHAMP_PATH="$(pwd)"
PREFETCH="scooby"
OUTPUT_DIR="${CHAMP_PATH}/out/scooby_perceptron"
...
while read TRACE; do
    ...
    echo "./experiments/champsim.py run ${TRACE_PATH}/${TRACE} --l2c-pref ${PREFETCH} --results-dir ${OUTPUT_DIR} --run-name ${TRACE} --extra-knobs \"‘--le_featurewise_pooling_type=1 --scooby_gamma=0.0 --scooby_enable_featurewise_engine=false’\"" >> ${SCRIPT_FILE}
    chmod +x ${SCRIPT_FILE}
