...
#ifdef DEBUG
#define debug_cout cerr << "[TRIAGE_CORE] "
#else
#define debug_cout if (0) cerr
#endif


TriageCore::TriageCore()
{
    trigger_count = 0;
    predict_count = 0;
    same_addr = 0;
...
void TriageCore::set_conf(TriageConfig *config)
{
    assert(config != NULL);
...
void TriageCore::train(uint64_t pc, uint64_t addr, bool cache_hit)
{
    if(cache_hit) {
...
void TriageCore::predict(uint64_t pc, uint64_t addr, bool cache_hit)
{
    uint64_t next_addr;
...
void TriageCore::calculatePrefetch(uint64_t pc, uint64_t addr,
...
uint32_t TriageCore::get_assoc()
{
    return on_chip_data.get_assoc();
}

void TriageCore::print_stats()
{
