...
void ScoobyDouble::init_knobs()
{
	SDActions.resize(knob::scooby_max_actions, 0);
	std::copy(knob::scooby_actions.begin(), knob::scooby_actions.end(), SDActions.begin());
	assert(SDActions.size() == knob::scooby_max_actions);
	assert(SDActions.size() <= MAX_ACTIONS);
...
void ScoobyDouble::init_stats()
{
	bzero(&stats, sizeof(stats));
	stats.predict.action_dist.resize(knob::scooby_max_actions*2, 0);
	stats.predict.issue_dist.resize(knob::scooby_max_actions*2, 0);
	stats.predict.pred_hit.resize(knob::scooby_max_actions*2, 0);
	stats.predict.out_of_bounds_dist.resize(knob::scooby_max_actions*2, 0);
	state_action_dist.clear();
}

ScoobyDouble::ScoobyDouble(string type) : Prefetcher(type)
{
	init_knobs();
	init_stats();
	...
	if(knob::scooby_enable_featurewise_engine)
	{
		brain_featurewise = new LearningEngineFeaturewise(this,
														...
														knob::scooby_max_actions * 2, // Action i has entry i : low_conf, i + len(SDActions): high_confidence
														knob::scooby_seed,
														knob::scooby_policy,
...
ScoobyDouble::~ScoobyDouble()
{
	if(brain_featurewise) delete brain_featurewise;
	if(brain) 		delete brain;
}

void ScoobyDouble::print_config()
{
	cout << "scooby_alpha " << knob::scooby_alpha << endl
		...
		<< "scooby_actions " << array_to_string(SDActions) << endl
		<< "scooby_max_actions " << knob::scooby_max_actions << " (" << knob::scooby_max_actions*2 << " with confidence)" <<endl
...
void ScoobyDouble::invoke_prefetcher(uint64_t pc, uint64_t address, uint8_t cache_hit, uint8_t type, vector<uint64_t> &pref_addr, vector<uint64_t> &pref_level)
{
...
void ScoobyDouble::update_global_state(uint64_t pc, uint64_t page, uint32_t offset, uint64_t address)
{
	/* @rbera TODO: implement */
}

Scooby_STEntry* ScoobyDouble::update_local_state(uint64_t pc, uint64_t page, uint32_t offset, uint64_t address)
{
	stats.st.lookup++;
...
uint32_t ScoobyDouble::predict(uint64_t base_address, uint64_t page, uint32_t offset, State *state, vector<uint64_t> &pref_addr, vector<uint64_t> &pref_level)
{
	...
	if (knob::scooby_enable_featurewise_engine)
	{
		...
		{
			pref_degree = get_dyn_pref_degree(max_to_avg_q_ratio, page, SDActions[action_index % SDActions.size()]);
		}
	...
	assert(action_index < knob::scooby_max_actions*2);

	MYLOG("act_idx %u act %d", action_index, SDActions[action_index % SDActions.size()]);

	uint64_t addr = 0xdeadbeef;
	Scooby_PTEntry *ptentry = NULL;
	int32_t predicted_offset = 0;
	if(SDActions[action_index % SDActions.size()] != 0)
	{
		predicted_offset = (int32_t)offset + SDActions[action_index % SDActions.size()];
		if(predicted_offset >=0 && predicted_offset < 64) /* falls within the page */
		{
			...
			if(new_addr)
			{
				pref_addr.push_back(addr); // Prefetch address
                pref_level.push_back(is_high_confidence(action_index) ? 0 : FILL_LLC); // Prefetch level: 0 = default (high confidence) or LLC (low confidence.)

                if (is_high_confidence(action_index))
                    stats.confidence.high_conf++;
                else stats.confidence.low_conf++;
                
				track_in_st(page, predicted_offset, SDActions[action_index % SDActions.size()]);
				stats.predict.issue_dist[action_index]++;
				if(pref_degree > 1)
				{
					gen_multi_degree_pref(page, offset, action_index, action_value, pref_degree, pref_addr, pref_level);
				}
				stats.predict.deg_histogram[pref_degree]++;
				ptentry->consensus_vec = consensus_vec;
                ptentry->high_confidence = is_high_confidence(action_index);
			}
			else
			{
 ...
 * false otherwise */
bool ScoobyDouble::track(uint64_t address, State *state, uint32_t action_index, Scooby_PTEntry **tracker)
{
	MYLOG(
        "addr@%lx state %x act_idx %u act %d confidence %d", 
        address, state->value(), action_index, SDActions[action_index % SDActions.size()], is_high_confidence(action_index)
    );
	stats.track.called++;
	...
	if(prefetch_tracker.size() >= knob::scooby_pt_size)
	{
		...
		MYLOG(
            ...
            ptentry->state->value(), ptentry->action_index, SDActions[ptentry->action_index % SDActions.size()], is_high_confidence(action_index)
        );
		if(last_evicted_tracker)
		{
			MYLOG(
                "last_victim_state %x last_victim_act_idx %u last_victim_act %d confidence %d", 
                last_evicted_tracker->state->value(), last_evicted_tracker->action_index, SDActions[last_evicted_tracker->action_index % SDActions.size()], is_high_confidence(action_index)
            );
			/* train the agent */
...
void ScoobyDouble::gen_multi_degree_pref(uint64_t page, uint32_t offset, uint32_t action_index, float action_value, uint32_t pref_degree,
                                   vector<uint64_t> &pref_addr, vector<uint64_t> &pref_level)
{
	stats.predict.multi_deg_called++;
	uint64_t addr = 0xdeadbeef;
	int32_t predicted_offset = 0;
    int32_t action = SDActions[action_index % SDActions.size()];
	if(action != 0)
	{
		for(uint32_t degree = 2; degree <= pref_degree; ++degree)
		{
			...
			if(predicted_offset >=0 && predicted_offset < 64)
			{
				...
				MYLOG("degree %u pred_off %d pred_addr %lx confidence %d", degree, predicted_offset, addr, is_high_confidence(action_index));
...
uint32_t ScoobyDouble::get_dyn_pref_degree(float max_to_avg_q_ratio, uint64_t page, int32_t action)
{
...
void ScoobyDouble::reward(uint64_t address)
{
	...
	for(uint32_t index = 0; index < ptentries.size(); ++index)
	{
		...
		MYLOG(
            "PT hit. state %x act_idx %u act %d confidence %d", 
            ptentry->state->value(), ptentry->action_index, SDActions[ptentry->action_index % SDActions.size()],
            is_high_confidence(ptentry->action_index)
        );
		...
		{
			assign_reward(ptentry, (is_high_confidence(ptentry->action_index) ? RewardType::correct_timely : RewardType::correct_timely_lowconf));
			MYLOG("assigned reward correct_timely(%d)", ptentry->reward);
		}
		else
		{
			assign_reward(ptentry, (is_high_confidence(ptentry->action_index) ? RewardType::correct_untimely : RewardType::correct_untimely_lowconf));
...
void ScoobyDouble::reward(Scooby_PTEntry *ptentry)
{
	MYLOG(
        "reward PT evict %lx state %x act_idx %u act %d confidence %d", 
        ptentry->address, ptentry->state->value(), ptentry->action_index, SDActions[ptentry->action_index % SDActions.size()],
        is_high_confidence(ptentry->action_index)
    );
	...
	{
		assign_reward(ptentry, (is_high_confidence(ptentry->action_index) ? RewardType::incorrect : RewardType::incorrect_lowconf));
		MYLOG("assigned reward incorrect(%d)", ptentry->reward);
	}
	ptentry->has_reward = true;
}

void ScoobyDouble::assign_reward(Scooby_PTEntry *ptentry, RewardType type)
{
	MYLOG(
...
int32_t ScoobyDouble::compute_reward(Scooby_PTEntry *ptentry, RewardType type)
{
...
void ScoobyDouble::train(Scooby_PTEntry *curr_evicted, Scooby_PTEntry *last_evicted)
{
	MYLOG("victim %s %u %d last_victim %s %u %d", curr_evicted->state->to_string().c_str(), curr_evicted->action_index, SDActions[curr_evicted->action_index % SDActions.size()],
												last_evicted->state->to_string().c_str(), last_evicted->action_index, SDActions[last_evicted->action_index % SDActions.size()]);
...
void ScoobyDouble::register_fill(uint64_t address)
{
	...
	if(!ptentries.empty())
	{
		...
		for(uint32_t index = 0; index < ptentries.size(); ++index)
		{
            // if (!is_high_confidence(ptentries[index]->action_index)) {
            //    cout << "[DEBUG] [register_fill] LOW CONFIDENCE ptentry found: addr=" << hex << address 
            //         << " action=" << dec << SDActions[ptentries[index]->action_index % SDActions.size()]
            ...
            stats.register_fill.set_conf[is_high_confidence(ptentries[index]->action_index)]++;
			ptentries[index]->is_filled = true;
			MYLOG(
                "fill PT hit. pref with act_idx %u act %d confidence %d", 
                ptentries[index]->action_index, SDActions[ptentries[index]->action_index % SDActions.size()],
                is_high_confidence(ptentries[index]->action_index),
            );
		}
	}
}

void ScoobyDouble::register_llc_fill(uint64_t address)
{
	...
	if(!ptentries.empty())
	{
		...
		for(uint32_t index = 0; index < ptentries.size(); ++index)
		{
            ...
            stats.register_llc_fill.set_conf[is_high_confidence(ptentries[index]->action_index)]++;
			ptentries[index]->is_filled = true;
			MYLOG(
...
void ScoobyDouble::register_prefetch_hit(uint64_t address)
{
...
vector<Scooby_PTEntry*> ScoobyDouble::search_pt(uint64_t address, bool search_all)
{
...
void ScoobyDouble::update_stats(uint32_t state, uint32_t action_index, uint32_t pref_degree)
{
	...
	else
	{
		...
		act_dist.resize(knob::scooby_max_actions*2, 0);
		act_dist[action_index]++;
		state_action_dist.insert(std::pair<uint32_t, vector<uint64_t> >(state, act_dist));
	}
}

void ScoobyDouble::update_stats(State *state, uint32_t action_index, uint32_t degree)
{
	...
	{
		it->second[action_index]++;
		it->second[knob::scooby_max_actions*2]++; /* counts total occurences of this state */
	}
	else
	{
		vector<uint64_t> act_dist;
		act_dist.resize((knob::scooby_max_actions*2)+1, 0);
		act_dist[action_index]++;
		act_dist[knob::scooby_max_actions*2]++; /* counts total occurences of this state */
		state_action_dist2.insert(std::pair<string, vector<uint64_t> >(state_str, act_dist));
	}

	auto it2 = action_deg_dist.find(action_index);
	...
	else
	{
		...
		deg_dist[degree]++;
		action_deg_dist.insert(std::pair<int32_t, vector<uint64_t>>(action_index, deg_dist));
	}
}
...
void ScoobyDouble::track_in_st(uint64_t page, uint32_t pred_offset, int32_t pref_offset)
{
	auto st_index = find_if(signature_table.begin(), signature_table.end(), [page](Scooby_STEntry *stentry){return stentry->page == page;});
	if(st_index != signature_table.end())
	{
...
void ScoobyDouble::update_bw(uint8_t bw)
{
...
void ScoobyDouble::update_ipc(uint8_t ipc)
{
...
void ScoobyDouble::update_acc(uint32_t acc)
{
...
bool ScoobyDouble::is_high_bw()
{
	return bw_level >= knob::scooby_high_bw_thresh ? true : false;
}

void ScoobyDouble::dump_stats()
{
	...
	for(uint32_t index = 0; index < SDActions.size(); ++index)
	{
		cout << "scooby_predict_action_" << SDActions[index] << " " << stats.predict.action_dist[index] << endl;
		cout << "scooby_predict_issue_action_" << SDActions[index] << " " << stats.predict.issue_dist[index] << endl;
		cout << "scooby_predict_hit_action_" << SDActions[index] << " " << stats.predict.pred_hit[index] << endl;
		cout << "scooby_predict_out_of_bounds_action_" << SDActions[index] << " " << stats.predict.out_of_bounds_dist[index] << endl;
	}
	...
	if(knob::scooby_enable_state_action_stats)
	{
		if(knob::scooby_enable_featurewise_engine)
		{
			...
			for (auto itr = state_action_dist2.begin(); itr != state_action_dist2.end(); ++itr)
			    pairs.push_back(*itr);
			sort(pairs.begin(), pairs.end(), [](std::pair<string, vector<uint64_t>>& a, std::pair<string, vector<uint64_t>>& b){return a.second[knob::scooby_max_actions*2] > b.second[knob::scooby_max_actions*2];});
	...
	{
		cout << "scooby_action_" << SDActions[it->first % SDActions.size()] << (is_high_confidence(it->first) ? "_high" : "_low") << "_deg_dist ";
	...
	for(uint32_t action_index = 0; action_index < SDActions.size()*2; ++action_index)
	{
        cout << "scooby_reward_" << SDActions[action_index % SDActions.size()] << (is_high_confidence(action_index) ? "_high " : "_low ");
		for(uint32_t reward = 0; reward < RewardType::num_rewards; ++reward) {
			cout << stats.reward.dist[action_index][reward] << ",";
		}
		cout << endl;
	}
